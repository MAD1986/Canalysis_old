...
%Run time:
for i=1:size(run_epochs_idx,1)
run_time_int{i}=run_time(run_epochs_idx(i,1):run_epochs_idx(i,2));
end
runtime=cell2mat((run_time_int)');

%No run time:
norun_epochs_idx=[[1;run_epochs_idx(1:end-1,2)] run_epochs_idx(:,1)];
 no_run_idx=run_idx(norun_epochs_idx);
 if no_run_idx(end,2)<=length(Cdf_time) ==1,
no_run_idx=[ [no_run_idx(:,1) ;no_run_idx(end,2)+1] [no_run_idx(:,2); length(Cdf_time) ]];
 end   
for i=1:size(norun_epochs_idx,1)
 no_run_time_int{i}=Cdf_time(norun_epochs_idx(i,1):norun_epochs_idx(i,2));
end
noruntime=cell2mat(( no_run_time_int)');
...
for i=1:size(onset_offset,2)
    for ii=1:size(run_epochs_idx,1)
onset=onset_offset{i}(:,1);
on_R{i}(:,ii)=onset>=run_epochs_idx(ii,1) & onset<=run_epochs_idx(ii,2);
