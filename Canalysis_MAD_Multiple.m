...
%nb of session
[Behavior{i}] = behavior_multiple(CSV{i}, Behavior{i});
...
for i=1:2
Imaging{i}.options.baselinesub.windwith=100; %width for the shifting window (in frame).
...
Events{i}.options.mindur=0.5; %Minimum duration (s) an event must stay above the ON threshold 
Events{i}.options.mergdur=0; %Merge consecutive epochs separated by (s)
Events{i}.options.mindurevent=1; %Min duration of event to be considered  %Danielson et al. used > 1s
tic;
[Events{i}]=eventdetect_V5(Events{i}, Imaging{i});
toc;
end
%Figure:
figure;hold on;
plot(Imaging{i}.trace_restricted_baselinesub(:,26));
plot(Events{i}.onset_binary(:,26));
...
tic;
[EventsProperties{i}{c}, NetworkProperties{i}{c}, PCAProperties{i}{c}]=transients_analysis_V8(onset_offset_all{i}(c,:), time{i}(c), Events{i}, Imaging{i});
toc;
end
end
...
%Where to save:
tic;
save(strcat(directory_name,'/Analysis/ExpDff_3SD/','ALL_analysed'), '-v7.3');
toc;
